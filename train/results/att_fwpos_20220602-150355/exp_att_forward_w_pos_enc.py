...
from model.model_conv import conv1d_mlp, conv1d_past, conv1d_att_w_pos_fwd
from model.model_conv_with_attention import Conv1dWithAttention
...
class prediction():
    def __init__(self,
        ...
        self.current_folder_path = str(pathlib.Path(__file__).parent.resolve()) + '/'
        self.file_prefix = self.current_folder_path + '../data/hatci_clinic/1st_TD_matched/dataset_1/'

        # results path
        self.starttime = datetime.now()
        self.results_path = self.current_folder_path + results_path + '/' +'att_fwpos_' + self.starttime.strftime("%Y%m%d-%H%M%S")
        ...
        shutil.copy('./exp_att_forward_w_pos_enc.py', self.results_path+'/exp_att_forward_w_pos_enc.py')
        ...
        with open(self.results_path + '/hyperparams.txt', 'w') as f:
            ...
            if self.multi_gpu:
                with self.strategy.scope():
                    self.model = conv1d_att_w_pos_fwd(past_filters, past_sizes, past_strides, forward_filters, forward_sizes,
                                                forward_strides)
    ...
    def result_plot(self, x_batch, y_batch, outputs):
        ...
        for i in range(10):
            ...
            plt.plot((np.arange(self.past_stepsize*(self.past_windowsize-1)+self.pred_stepsize,
                                self.past_stepsize*(self.past_windowsize-1)+self.pred_stepsize*self.pred_windowsize+1,
...
#         self.pos_enc_past = tf.eye(self.pred_windowsize, self.pred_windowsize, batch_shape=[self.batch_size])
        self.pos_enc_past = tf.tile(tf.expand_dims(self.pos_encoder(np.arange(10), 40), axis=0), [self.batch_size, 1, 1])
#         self.pos_enc = tf.tile(tf.expand_dims(tf.range(10), axis=0), [self.batch_size, 1])
        ...
        if self.run_test:
            train_dataset = tf.data.Dataset.from_tensor_slices(
                (train_past[:self.global_batch_size], train_map[:self.global_batch_size], train_pred[:self.global_batch_size])).batch(
                self.global_batch_size)
            valid_dataset = tf.data.Dataset.from_tensor_slices(
                (valid_past[:self.global_batch_size], valid_map[:self.global_batch_size], valid_pred[:self.global_batch_size])).batch(
                self.global_batch_size)
        else:
            train_dataset = tf.data.Dataset.from_tensor_slices((train_past, train_map, train_pred)).shuffle(
                ...
                self.global_batch_size, drop_remainder=True, num_parallel_calls=tf.data.AUTOTUNE).prefetch(2).repeat(self.epochs)
        ...
        earlystopper = tk.callbacks.EarlyStopping(monitor='valid_loss', patience=self.patience, verbose=1, mode='min', restore_best_weights=True)
...
#             pos_enc_past = tf.eye(self.pred_windowsize, self.pred_windowsize, batch_shape=[len(tupids)])
            pos_enc_past = tf.tile(tf.expand_dims(self.pos_encoder(np.arange(10), 40), axis=0), [len(tupids), 1, 1])
#             pos_enc = tf.tile(tf.expand_dims(tf.range(10), axis=0), [len(tupids), 1])
...
vel_predictor = prediction(batch_size=2000,
                           lr=5e-4,
                           epochs=2000,
                           patience=20,
                           log_freq=200,
...
#                            forward_dilation_rates=[2, 2, 2, 2, 2 ,2, 2, 2, 2, 2, 1],
                           results_path='./results',
                           run_test=False)
